...
double accuracy(image* test_img, byte* test_label, unsigned datasize, unsigned minibatch_size, ann_t *nn)
{
    ...
    double *var = (double *)malloc(10 * minibatch_size * sizeof(double));
    ...
    {        
        ...
        forward(nn, sigmoid);
        cudaMemcpy(var, nn->layers[nn->number_of_layers - 1]->activations->m, 10 * minibatch_size * sizeof(double), cudaMemcpyDeviceToHost);
